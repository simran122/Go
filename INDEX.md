...
**Go version:** This guide is written for **current Go** (e.g. Go 1.22+). Examples use features that are stable in recent releases. For the latest official docs and API changes, see [go.dev/doc](https://go.dev/doc) or use **Context7 MCP** (if available) to fetch up-to-date library documentation.
