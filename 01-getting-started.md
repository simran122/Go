...
**What is Go?**  
Go (also called Golang) is a programming language made by Google. It is easy to read, fast to run, and great for building software that does many things at once (concurrency). You write code in **`.go`** files, then run them with the **`go`** command.
...
4. When it’s done, open a **new** terminal and type:
   ...
   You should see something like: `go version go1.25.x ...` (the exact number depends on your install; e.g. **go1.25.0** or newer).
